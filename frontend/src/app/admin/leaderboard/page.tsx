...
import { Trophy, Medal, ClipboardCheck, Smile, GraduationCap, FileText, User } from 'lucide-react';
                                    ...
                                    return (
                                        <div key={actualItem.id}
                                            ...
                                            <div className="w-12 h-12 rounded-full mx-auto mb-2 bg-surface-elevated border-2 border-border flex items-center justify-center overflow-hidden">
                                                {actualItem.avatarUrl ? (
                                                    <img src={actualItem.avatarUrl} alt="" className="w-full h-full object-cover" />
                                                ) : (
                                            ...
                                            <p className="text-base font-black mt-2" style={{ color: activeTabInfo.color }}>
                                                {getValueLabel(actualItem)}
                                            </p>
                                            {getSubLabel(actualItem) && (
                                                <p className="text-[10px] text-text-muted mt-0.5">{getSubLabel(actualItem)}</p>
                                            )}
                                        </div>
                                    );
                                })}
                                    ...
                                    return (
                                        <div key={item.id} className="flex items-center gap-4 px-5 py-3 hover:bg-surface-elevated/50 transition-colors">
                                            ...
                                            <div className="text-right shrink-0">
                                                <p className="text-sm font-bold" style={{ color: activeTabInfo.color }}>
                                                    {getValueLabel(item)}
                                                </p>
                                                {getSubLabel(item) && (
                                                    <p className="text-[10px] text-text-muted">{getSubLabel(item)}</p>
                                                )}
                                            </div>
                                        </div>
                                    );
