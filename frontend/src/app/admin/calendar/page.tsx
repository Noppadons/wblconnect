    ...
    return (
        ...
        >
                ...
                <div className="space-y-5">
                    ...
                    <div className="card p-4">
                        <div className="grid grid-cols-7 gap-px mb-1">
                            {DAYS_TH.map((d, i) => (
                                <div key={i} className={`text-center text-[11px] font-bold py-2 ${i === 0 ? 'text-danger' : 'text-text-muted'}`}>
                                    {d}
                                </div>
                            ))}
                        </div>
                        <div className="grid grid-cols-7 gap-px">
                            ...
                            {Array.from({ length: daysInMonth }).map((_, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        <span className={`text-[12px] font-bold inline-flex items-center justify-center w-6 h-6 rounded-full ${isToday ? 'bg-primary text-white' : 'text-text-secondary'}`}>
                                            {day}
                                        </span>
                                        <div className="mt-0.5 space-y-0.5">
                                            ...
                                            {dayEvents.length > 3 && (
                                                <span className="text-[10px] text-text-muted">+{dayEvents.length - 3} อื่นๆ</span>
                                            )}
                                        </div>
                                    </div>
                                );
                    ...
                    {selectedDate && (
                        <div className="card p-5">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <button onClick={() => setSelectedDate(null)} className="p-1 text-text-muted hover:text-text-primary">
                                    <X size={14} />
                                </button>
                            </div>
                                        ...
                                        return (
                                            <div key={e.id} className="flex items-start gap-3 p-3 rounded-xl bg-surface-elevated border border-border">
                                                ...
                                                <div className="flex gap-1">
                                                    <button onClick={() => handleEdit(e)} className="p-1.5 text-text-muted hover:text-primary rounded-lg hover:bg-primary/10 transition-colors">
                                                        <Edit2 size={13} />
                                                    </button>
                                                    <button onClick={() => handleDelete(e.id)} className="p-1.5 text-text-muted hover:text-danger rounded-lg hover:bg-danger/10 transition-colors">
                                                        <Trash2 size={13} />
                                                    </button>
                                                </div>
                                            </div>
            ...
            <EventFormModal
                ...
                event={editingEvent}
                onSuccess={() => { setShowModal(false); setEditingEvent(null); fetchEvents(); }}
            />
        </AppShell>
    );
}
...
function EventFormModal({ isOpen, onClose, event, onSuccess }: {
    ...
    const [form, setForm] = useState({
        ...
        type: 'ACTIVITY' as EventType, allDay: true, color: '',
    });
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })}
                        className="input-field w-full" placeholder="เช่น สอบกลางภาค" />
                </div>
                <div>
                    <label className="text-xs font-semibold text-text-secondary mb-1.5 block">รายละเอียด</label>
                    <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                        className="input-field w-full" rows={3} placeholder="รายละเอียดเพิ่มเติม..." />
                </div>
                ...
                <div className="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" disabled={saving} className="btn-primary h-9 px-4 text-[13px]">
                        {saving ? 'กำลังบันทึก...' : event ? 'บันทึก' : 'เพิ่มกิจกรรม'}
                    </button>
                </div>
            </form>
        </Modal>
