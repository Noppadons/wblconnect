...
export default function StudentSchedulePage() {
    ...
    const totalSubjects = new Set(schedules.map(s => s.subject?.id)).size;

    return (
        <AppShell role="STUDENT" sidebarItems={STUDENT_SIDEBAR} user={user}
            pageTitle="ตารางเรียน"
            pageSubtitle={`${schedules.length} คาบ/สัปดาห์ • ${totalSubjects} วิชา`}
        >
            {loading ? (
                ...
                <div className="py-16 text-center card">
                    ...
                    <p className="text-text-secondary font-semibold">ยังไม่มีตารางเรียน</p>
                    <p className="text-sm text-text-muted mt-1">กรุณาติดต่อครูที่ปรึกษาหรือผู้ดูแลระบบ</p>
                </div>
            ) : (
                <>
                    {/* Today's Schedule */}
                                    ...
                                    return (
                                        <div key={s.id} className={`p-3 rounded-xl border-2 transition-all ${isCurrent ? 'border-primary bg-primary/5 shadow-sm' : 'border-border bg-white'}`}>
                                            ...
                                            <div className="flex items-center justify-between mt-1.5">
                                                ...
                                                {s.teacher && (
                                                    <span className="text-xs text-text-muted truncate ml-2">ครู{s.teacher.user?.firstName}</span>
                                                )}
                                            </div>
                                        </div>
                                    );
                    ...
                    <div className="card p-5">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-base font-semibold text-text-primary flex items-center gap-2">
                                <BookOpen size={18} /> ตารางเรียนรายสัปดาห์
                            </h3>
