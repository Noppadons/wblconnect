...
export class AnalyticsService {
    ...
    async getStudentAiInsights(studentId: string) {
        ...
        const behaviorScore = this.calculateBehaviorScore(student.attendance, student.behaviorLogs);
        const recommendations = this.generateRecommendations(student.submissions, student.attendance);
    ...
    private generateRecommendations(submissions: any[], attendance: any[]) {
        ...
        if (rate < 80) {
            recommendations.push({
                ...
                advice: 'ควรปรับปรุงการเข้าเรียนให้สม่ำเสมอกว่านี้ เพื่อไม่ให้พลาดเนื้อหาสำคัญ',
                priority: 'HIGH'
            });
        }

        // Subject-specific performance
        const subjectScores: Record<string, { points: number; max: number; name: string }> = {};
        ...
        Object.values(subjectScores).forEach(score => {
            if (score.max > 0 && (score.points / score.max) < 0.5) {
                recommendations.push({
                    subject: score.name,
                    advice: 'คะแนนในวิชานี้ค่อนข้างต่ำ ควรทบทวนเนื้อหาเพิ่มเติมหรือปรึกษาครูผู้สอน',
                    priority: 'MEDIUM'
                });
            }
        });
