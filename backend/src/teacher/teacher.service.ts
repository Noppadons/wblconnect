...
export class TeacherService {
    ...
    async getDashboardStats(userId: string) {
        ...
        const stats = {
            ...
            className: teacher.homeroomClass ? `${teacher.homeroomClass.grade?.level || ''}/${teacher.homeroomClass.roomNumber}` : 'ไม่มีที่ปรึกษา'
        };
    ...
    async getMyStudents(userId: string) {
        ...
        return this.prisma.student.findMany({
            ...
            include: {
                ...
                behaviorLogs: { // Assuming relation exists, if not need to add
                    select: { points: true }
                }
            },
    ...
    async addBehaviorScore(userId: string, studentId: string, points: number, type: string, content: string) {
        // Verify teacher
        const teacher = await this.prisma.teacher.findUnique({ where: { userId } });
