...
export class AdminService {
    ...
    async getDashboardStats() {
        ...
        const [totalStudents, totalTeachers, totalClassrooms, attendanceToday] = await Promise.all([
    ...
    async getDashboardCharts() {
        ...
        const students = await this.prisma.student.findMany({ select: { gpa: true } });
        const ranges = [
    ...
    async createStudent(data: any) {
        const { email, password, firstName, lastName, studentCode, classroomId, avatarUrl } = data;
    ...
    async updateStudent(id: string, data: any) {
        ...
        if (data.parentLineToken !== undefined) updateData.parentLineToken = data.parentLineToken || null;
    ...
    async getSemesterSummary(classroomId?: string) {
        ...
        const studentSummaries = students.map((s) => {
            ...
            const gradedSubjects = s.enrolledSubjects.filter((es) => es.grade !== null);
            const totalCredits = gradedSubjects.reduce((sum, es) => sum + (es.subject?.credit || 1.5), 0);
            const weightedSum = gradedSubjects.reduce((sum, es) => sum + (es.grade || 0) * (es.subject?.credit || 1.5), 0);
            const gpa = totalCredits > 0 ? Math.round((weightedSum / totalCredits) * 100) / 100 : s.gpa || 0;
            ...
            const attendanceRate = totalAttendance > 0 ? Math.round((present + late) / totalAttendance * 10000) / 100 : 0;

            // Behavior
            const positivePts = s.behaviorLogs.filter((b) => b.type === 'POSITIVE').reduce((sum, b) => sum + b.points, 0);
            const negativePts = s.behaviorLogs.filter((b) => b.type === 'NEGATIVE').reduce((sum, b) => sum + Math.abs(b.points), 0);
            ...
            const avgScore = graded.length > 0
                ? Math.round(graded.reduce((sum, sub) => sum + ((sub.points || 0) / (sub.assignment?.maxPoints || 100)) * 100, 0) / graded.length)
                : 0;
            ...
            if (gradedSubjects.some((es) => (es.grade || 0) < 1.0)) risks.push('มีวิชาที่ไม่ผ่าน');
            ...
            return {
                ...
                attendance: { total: totalAttendance, present, late, absent, leave, rate: attendanceRate },
                behavior: { positive: positivePts, negative: negativePts, score: behaviorScore },
        ...
        const avgGpa = totalStudents > 0 ? Math.round(studentSummaries.reduce((s, st) => s + st.gpa, 0) / totalStudents * 100) / 100 : 0;
        const avgAttendanceRate = totalStudents > 0 ? Math.round(studentSummaries.reduce((s, st) => s + st.attendance.rate, 0) / totalStudents * 100) / 100 : 0;
        const avgBehaviorScore = totalStudents > 0 ? Math.round(studentSummaries.reduce((s, st) => s + st.behavior.score, 0) / totalStudents * 100) / 100 : 0;
        ...
        return {
            overview: { totalStudents, avgGpa, avgAttendanceRate, avgBehaviorScore, atRiskCount, gpaDistribution },
