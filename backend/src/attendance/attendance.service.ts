...
export class AttendanceService {
    ...
    async checkAttendance(data: { studentId: string; status: AttendanceStatus; period: number; remarks?: string; date?: string }) {
        ...
        if (data.date) {
            ...
            const now = new Date();
            targetDate = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Bangkok' }));
        }
        ...
        try {
            ...
            if (shouldNotify && message) {
                ...
                if (!notified) {
                    console.log(`[Attendance] No specific token found for ${name}, skipping notification`);
                }
            }
        } catch (err) {
